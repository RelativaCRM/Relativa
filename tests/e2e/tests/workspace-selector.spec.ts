...
test.describe('Workspace Selector', () => {
  ...
  test('user with multiple workspaces is redirected to /workspace-select after login', async ({ page }) => {
  ...
  test('clicking a workspace card redirects to home', async ({ page }) => {
    await clearSession(page);
    await fillLogin(page, MULTI_EMAIL, FRESH_PASS);
    await expect(page).toHaveURL(/\/workspace-select/, { timeout: 10000 });
  ...
  test('user with single workspace is auto-selected and lands on home', async ({ page }) => {
    await clearSession(page);
  ...
  test('create workspace from selector redirects to home', async ({ page }) => {
    await clearSession(page);
    await fillLogin(page, MULTI_EMAIL, FRESH_PASS);
    await expect(page).toHaveURL(/\/workspace-select/, { timeout: 10000 });
