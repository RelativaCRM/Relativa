...
> **Last verified:** 2026-04-28 (CR-138 + post-review fixes: EntitiesView list, scoped 401 auto-clearSession, profile re-fetch in router guard, ws role labels for ws_manager/ws_analyst)
...
- **Routes:** `/workspaces/:id/entities` → `EntitiesView.vue` (name `workspace-entities`); `/workspaces/:id/entities/new` → `EntityCreateForm.vue` (name `workspace-entity-create`).
