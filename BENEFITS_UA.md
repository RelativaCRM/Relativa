...
## 🔐 Дворівневий контроль доступу на основі ролей (RBAC) — 💎 Growth

*Технологія: Перегляд ролей на рівні організації та робочого простору, застосований через RBAC-middleware (`myPermissions` вбудовано у відповіді workspace/organization)*
...
- 🤝 **Незалежні ролі організації та робочого простору** — sales-команди, які ведуть кілька пайплайнів у різних робочих просторах, отримують гнучке управління доступом без конфліктів дозволів

---

## 📋 Розширений журнал аудиту — ♾️ Enterprise

*Технологія: Audit Log з 13 параметрами фільтрації (ініціатор, ціль, діапазон дат, робочий простір, тип дії тощо) — `GET /audit-log`*

- 📋 **Кожна дія — назавжди зафіксована** — хто, що змінив, коли і з якого робочого простору — повний пошук за два кліки
- ✅ **Готовність до compliance** — проходьте перевірки безпеки без паніки; фільтруйте журнал аудиту за датою, користувачем або сутністю за лічені секунди
...
| Функція | API-ендпоінт | Рівень | Сервіс |
|---------|-------------|--------|--------|
| ML-скорінг угод + відтоку | `POST /api/ml/score/batch` | 💎 Growth | ML Service |
| Візуалізація графу | `GET /api/v1/graph/` | 💎 Growth | Graph Service |
| Комбіноване створення в графі | `POST /api/v1/workspaces/{id}/entity-graph/create` | ♾️ Enterprise | Graph Service |
| Оновлення графу в реальному часі | `/graph/hubs/graph` (SignalR) | ♾️ Enterprise | Graph Service |
| Оновлення сутностей у реальному часі | `/core/hubs/core` (SignalR) | ♾️ Enterprise | Core Service |
| Розширений журнал аудиту | `GET /audit-log` | ♾️ Enterprise | Audit Service |
| Дашборд організації | `GET /api/v1/dashboard/summary` (+ 5 ендпоінтів) | 💎 Growth | Graph Service |
| Дашборд робочого простору | `GET /api/v1/dashboard/workspace/{id}/summary` (+ 5 ендпоінтів) | 💎 Growth | Graph Service |
