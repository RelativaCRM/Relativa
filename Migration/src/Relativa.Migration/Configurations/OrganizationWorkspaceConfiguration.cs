...
public class OrganizationWorkspaceConfiguration : IEntityTypeConfiguration<OrganizationWorkspace> {
    public void Configure(EntityTypeBuilder<OrganizationWorkspace> builder) {
        ...
        builder.HasOne(e => e.Organization).WithMany(o => o.OrganizationWorkspaces).HasForeignKey(e => e.OrgId).OnDelete(DeleteBehavior.Cascade).HasConstraintName("fk_ow_org");
        builder.HasOne(e => e.Workspace).WithMany(w => w.OrganizationWorkspaces).HasForeignKey(e => e.WorkspaceId).OnDelete(DeleteBehavior.Cascade).HasConstraintName("fk_ow_workspace");
    }
}
